...
void User::copyFrom(const User& other) {
	size_t size_name = strlen(other.name) + 1;
	this->name = new char[size_name];
	strcpy_s(this->name, size_name, other.name);

	strcpy_s(this->egn, EGN_MAX_LENGTH, other.egn);
	this->age = other.age;

	size_t size_pass = strlen(other.password) + 1;
	this->password = new char[size_pass];
	strcpy_s(this->password, size_pass, other.password);
}
void User::moveFrom(User&& other) {
	...
	for (size_t i = 0; i < EGN_MAX_LENGTH; i++)
	{
