...
void Employee::tasks() const {
	if (this->get_task_count() == 0) {
		...
		for (unsigned i = 0; i < this->get_task_count(); i++) {
			std::cout << "[" << tasks_pending[i].get_task_id() << "] " <<
				tasks_pending[i].convert_type_to_string().c_str() << " - " << 
				tasks_pending[i].get_task_name()<< std::endl;
		}
	}
}
...
void Employee::add_task(const Task& new_task) {
